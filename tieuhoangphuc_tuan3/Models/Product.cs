using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;
using Microsoft.AspNetCore.Mvc.ModelBinding.Validation;

namespace WebBanDienThoai.Models
{
    public class Product
    {
        public int Id { get; set; }

        [Required, StringLength(100)]
        public string Name { get; set; }

        [Range(0.01, 100000000.00)]
        public decimal Price { get; set; }

        public string Description { get; set; }

        public string? ImageUrl { get; set; }

        public List<ProductImage>? Images { get; set; }

        public int CategoryId { get; set; }
        public Category? Category { get; set; }

        public double Rating { get; set; }

        public int DiscountPercent { get; set; }

        public decimal DiscountedPrice { get; set; } // Để lưu giá sau giảm

        public int? SubCategoryId { get; set; }
        public SubCategory? SubCategory { get; set; }

        // --- CHỨC NĂNG 2: QUẢN LÝ STOCK CẢNH BÁO ---
        public int Quantity { get; set; }              // Số lượng hiện tại
        public int MinStockLevel { get; set; }          // Mức tồn kho tối thiểu (Ngưỡng cảnh báo)

        /// <summary>
        /// Thuộc tính logic tự động tính toán xem sản phẩm có đang sắp hết hàng hay không.
        /// [NotMapped] đảm bảo trường này không tạo cột dư thừa trong Database.
        /// </summary>
        [NotMapped]
        public bool IsLowStock => Quantity <= MinStockLevel;

        // -------------------------------------------

        // --- CHỨC NĂNG 1: ĐÁNH DẤU HOT / BEST-SELLER ---
        /// <summary>
        /// Cho phép Admin đánh dấu sản phẩm nổi bật để đưa ra trang chủ.
        /// </summary>
        public bool IsHot { get; set; } = false;
        // -----------------------------------------------

        public DateTime? LastImportDate { get; set; }  // Ngày nhập hàng gần nhất
        public DateTime? LastExportDate { get; set; }  // Ngày xuất hàng gần nhất

        [ValidateNever]
        public ProductSpecs? Specs { get; set; }

        [ValidateNever]
        public virtual ICollection<ProductVariant>? Variants { get; set; }

        public string? ServiceCommitment { get; set; }
    }
}